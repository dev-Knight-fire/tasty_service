...
const ManageListPage = () => {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      try {
        setIsLoading(true);
        const listsQuery = query(collection(db, "lists"), orderBy("createdAt", "desc"));
        const querySnapshot1 = await getDocs(listsQuery);
  ...
  const handleStatusChange = async (listingId, newStatus) => {
    ...
    const userDocRef = doc(db, "lists", listingId);
  ...
  const handleEditSubmit = async (e) => {
    ...
    setIsSubmiting(true);
    const userDocRef = doc(db, "lists", editFormData.id);
    await updateDoc(userDocRef, {
      status: editFormData.status,
    });

    const listDocRef = doc(db, "lists", editFormData.id);
    ...
    setIsEditModalOpen(false);
    toast.success(`Listing ${editFormData.id} has been updated`);
  };
  ...
  const renderStars = (rating) => {
    ...
    if (halfStar) {
      stars.push(
        <svg key="half" className="w-4 h-4 text-yellow-400 inline" fill="currentColor" viewBox="0 0 20 20">
          <defs>
          ...
          <path fill="url(#half-grad)" d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.967a1 1 0 00.95.69h4.175c.969 0 1.371 1.24.588 1.81l-3.38 2.455a1 1 0 00-.364 1.118l1.287 3.966c.3.922-.755 1.688-1.54 1.118l-3.38-2.454a1 1 0 00-1.175 0l-3.38 2.454c-.784.57-1.838-.196-1.54-1.118l1.287-3.966a1 1 0 00-.364-1.118L2.049 9.394c-.783-.57-.38-1.81.588-1.81h4.175a1 1 0 00.95-.69l1.286-3.967z"/>
        </svg>
      );
    }
                            ...
                            <div key={index} className="space-y-2">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  {renderStars(review.rating)} <b>{review.rating}</b>
                                </div>
                              </div>
                              <p className="text-sm text-gray-700 italic">
                                &quot;{review.text}&quot;
                              </p>
                              {index < selectedListing.reviews.length - 1 && (
                                <Separator />
                              )}
                            </div>
                          ))}
              ...
              <form onSubmit={handleEditSubmit}>
                ...
                <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                  <button
                    ...
                    className={`m-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-[#206645] text-white font-medium text-gray-700 hover:bg-green focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#206645] sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm ${
                      isSubmiting ? "opacity-75 cursor-not-allowed" : ""
                    }`}
                  >
                    {isSubmiting ? (
            ...
            <div className="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
              ...
              <div className="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
                <button
                  type="button"
                  onClick={() =>
                    handleStatusChange(selectedListing.id, "deleted")
                  }
