...
const ManageReviewsPage = () => {
      ...
      try {
        ...
        const reviewsData = await Promise.all(
          querySnapshot.docs.map(async (doc) => {
            ...
            // Fetch related listing/place data
            if (reviewData.listingId) {
              try {
                const listingSnap = await getDoc(doc(db, 'lists', reviewData.listingId));
                if (listingSnap.exists()) {
                  reviewData.listing = { id: listingSnap.id, ...listingSnap.data() };
                }
              } catch (listingErr) {
                console.error('Error fetching listing for review:', listingErr);
              }
            }
            ...
            return {
              ...
              listingName: reviewData.listing?.name || 'Unknown Listing',
              listingType: reviewData.listing?.entryType || 'Unknown',
            };
          })
        );
  ...
  }, [statusFilter, ratingFilter, searchQuery, reviews]);
  ...
  const handleStatusChange = async (reviewId, newStatus) => {
    try {
      ...
      toast.success(`Review ${reviewId} ${newStatus}!`);
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      const reviewDocRef = doc(db, "reviews", editFormData.id);
      await updateDoc(reviewDocRef, updatedData);

      const updatedReviews = reviews.map((review) =>
        review.id === editFormData.id ? { ...editFormData } : review
      );

      setReviews(updatedReviews);
      setIsEditModalOpen(false);
      toast.success(`Review ${editFormData.id} has been updated`);
    } catch (error) {
